...
`headline`、`public_explanation` 和 `promise` 为 v1/v2/v3 共用的 additive 字段：顶层 headline
是 15–60 字、单行且与最终 verdict 一致的首屏标题；public_explanation 固定包含
`headline/conclusion/narrative/audit_items/charge_facts/basis/clinical_evidence/review_needs`，后者为空或
...
界面应展示 headline 并允许展开完整 reasoning/evidence，严格 DTO 应将新增字段建模为可选并
允许未知字段。该增量不改变 v3 按实际收费源行展开 `matched_items[]`、兼容数组等长
对齐或每条规则一张 card 的语义。
...
headline 契约需要先对 SQLite `audit_runs` 与 SQL Server `javert_audit_runs` 幂等增加 nullable
