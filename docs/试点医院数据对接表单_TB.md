...
| 档 | 定位 | 表 | 数量 |
|----|------|----|------|
| **第 1 档 · 必备** | 系统跑起来的最低集合 | 文书 + 费用×2 + 诊断 + 手术 + 机构字典 + 就诊登记 | 7 张 |
...
诊断 + 手术补齐后覆盖全部 143 条已装载规则; 第 2 档补齐后检验指征类 + DRG 链路生效。
...
| **交付形式** | 三选一: ① 直接灌入我方 142 `TP_data_hub` 库 (SQL Server, 表已建好); ② 院内建同构库我方取数; ③ 按上述表结构导出 CSV (UTF-8) 交付 |
