...
## 15. 专家PDF导入纠正（2026-09-07 UTC）

第一版“技术可读”验收遗漏了导入完整性：费用/结算页被写入notes，仅三项目标费用被发布。本次功能提交1744d9d新增人工复核PDF导入门禁，按原页复核并替换目标病例数据，不能沿用§14中的31段/3条作为当前口径。

- 数据校验与真实源回读均为23段临床文书、62条费用、合计4263.82元，费用总账/结算正文在notes中为0；体温单2页正确分类，心电/曲线低可信文本明确待核。4项编码和2项遮挡名称不猜填。
- 更新前数据和SQLite备份在 `/home/admin2/backup/javert-eye-import-20260907-120528`；代码/.env/进程备份在 `/home/admin2/backup/javert-git-20260907-120522-2192976`。目录0700，文件0600。
...
原始身份显示纠正：用户明确不脱敏，故通过内部服务逐字核验原姓名/原就诊号，作为独立显示列保留。提交7a27353+04117c2修正所有身份标题并强制CSV字符串读取；字段扩展时保留旧列逐值一致。部署后原名/原号展示、20条结果不变、21页原文不变均通过；未重跑规则。修正前备份`/home/admin2/backup/javert-chronic-identity-20260908-154715`，最终安装备份`/home/admin2/backup/javert-git-20260908-155817-2545122`；临时身份清单已清理。
