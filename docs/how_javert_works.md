...
> 243真实数据接入：院区+首页SYXH → 唯一BAH/同卡小结 → 就诊JZLSH；
> 费用与文书按各自键取数后统一为内部首页号，ETL与工作台复用同一解析，歧义停止。
> 本次不改变规则或数据库schema；以下功能库存为历史快照，当前以CLI为准。
> 运维入口：[243单例与发布](243_release_cookbook.md)。
...
**核心价值**: 将人工核查 1 位患者需要数小时翻阅费用+病历的工作，压缩到 **平均 10 分钟自动完成**，且覆盖 **140+ 条审计规则** (8 套模板 M1-M8) 的全量扫描。
...
| 虚构医药服务 (M5) | 费用有而病历无对应操作记录 | 8 条 |
| 过度诊疗 (M6) | 无适应症的有创操作、超疗程用药 | 9 条 |
| 串换收费 (M7) | 低价项目套高价编码 | 20 条 |
| 药品违规 (M8) | 药品超说明书适应症 / 超医保限定支付 (限适应症/超说明书/限二线/禁忌症) | 33 条 |
| **合计** | | **143 条 ready** (M1-M8) |
...
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  ① 规则库 (143 条 YAML)   ② 模板库 (8 套 M1-M8)            │
│       ↓                         ↓                            │
│  ③ Router 预筛 ──→ 与本患者相关的规则子集                    │
│       ↓                                                      │
│  ④ LLM Agent 循环 (每条规则独立审计)                         │
│       │                                                      │
...
**关键设计**: LLM 不是盲目地跑完所有工具 — 它根据规则内容**自主决策**先查什么、再查什么。例如：
- 如果费用中根本没有"全身断层"相关条目 → **第 1 步就返回 CLEAN**，不浪费后续查询
- 如果费用命中但文书有合理解释 → 继续查文书再判定

### 3.3 七大审计工具

Javert 的 LLM 不直接读原始数据，而是通过**七个专用工具**按需查询:
...
| **drug_audit_lookup** (v0.8) | 患者用药 ∩ 928 药监管 KB + 病案首页诊断 (M8) | bulk(patient) → 命中药 + 限定/说明书依据 | 命中药 + 诊断 |
...
审 1 位患者不需要跑全部 143 条规则。Router 在 LLM 介入之前，用 **2 秒内**完成三层过滤:

```
第一层: 规则状态过滤
  143 条 ready → 去掉 abandoned/drafting → 143 条通过
...
结果: 143 条 → 平均约 35-45 条 (router 砍 ~68%)
```

### 3.5 规则模板体系

143 条规则不是逐条手写 — 而是用 **8 套共享模板 (M1-M8)** 批量生成:
...
- 所有数据存储在**医院内网服务器** (192.168.31.62)，不出外网
- LLM 模型部署在**同一内网 GPU 服务器**，无需调用外部 API
