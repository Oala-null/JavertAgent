...
日期：2026-08-18（2026-08-21 更新）

测试主机：62（NVIDIA RTX 5880 Ada 48GB）

结论口径：部署可行性与 Javert 适配评估，不是临床金标准验证

## 0. 2026-08-21 最终裁决 JSON 修复与十患者扩样（最新）

### 0.1 修复内容与真实门禁

截图所示问题不是函数工具未适配，而是 Qwen3.8 已完成工具调查后产出的最终裁决 JSON
不合法；旧 repair 仍携带 `tools`，并只靠自然语言要求 fenced JSON，因而可能二次失败并落
`INCONCLUSIVE/confidence=0`。本次保持调查轮不变，只在已有成功工具证据后的 repair、
工具总预算终局和 deadline 终局移除 tools，并使用 SGLang OpenAI-compatible
[`response_format.type=json_schema`](https://docs.sglang.io/docs/advanced_features/structured_outputs)
约束四个裁决字段。Qwen3.6 的 text 协议不走这条路径。

验证结果：

- provider 转发 schema、native malformed repair、预算终局等定向测试通过；完整套件
  **1181 passed / 1 skipped**。
- 62 上无患者数据的真实 schema 请求返回可直接解析的 JSON；128 tokens 会截断，512
  tokens 正常，因此保留 repair 512、budget-final 1024 的有界预算。
- 截图对应的 `R015` 用同一历史患者重跑：从旧
  `INCONCLUSIVE/confidence=0/malformed repair failed/4 tools/17.6s` 恢复为
  `CLEAN/confidence=0.90/4 tools/44.7s`，malformed=0。

### 0.2 十患者配对设计

- 从既有 `batch_50_new` 候选中排除已跑 `ab3.8` 的病例后，以固定随机种子
  `20260821` 抽取 10 个新患者；患者标识只保存在 62 的 mode 0600 临时清单中，不入 Git
  或报告。
- 两模型使用同一患者、同一 Router、同一 ready 规则快照、`batch_tag=ab3.8`、患者内并发
  2、患者间串行、temperature 0、thinking off、drift guard off。
- Qwen3.6 使用生产 text 工具协议和既有 runtime；Qwen3.8 使用 native tools 和隔离
  SGLang 0.5.17 runtime。该差异是两个模型各自可部署 bundle 的必要组成，不是同 runtime
  的纯权重消融。
- 两侧均完成 10/10 患者、340/340 规则，配对缺失 0；SQLite 和 142 各有 340 行，pending 0。

### 0.3 十患者结果

| 指标 | Qwen3.6-35B-A3B-FP8 | Qwen3.8-27B-FP8 | 3.8 / 3.6 |
|---|---:|---:|---:|
| 患者 / 规则 | 10 / 340 | 10 / 340 | 1.00 |
| V / I / C | 2 / 9 / 329 | 1 / 14 / 325 | — |
| JSON/截断等技术失败 | 1 | **0** | — |
| confidence=0 | 1 | **0** | — |
| 工具调用 | 1,560 | 1,198 | **0.768** |
| 累计规则时长 | 2,243.1s | 8,696.2s | **3.877×** |
| 单规则 p50 / p95 | 5.40s / 16.16s | 23.61s / 58.65s | 4.37× / 3.63× |
| 十患者墙钟合计 | 1,153.9s | 4,464.1s | **3.869×** |
| 单患者墙钟 p50 / p95 | 129.3s / 158.7s | 449.6s / 723.2s | 3.48× / 4.56× |

Qwen3.8 的 malformed 观测率为 0/340；若暂按规则独立的二项模型描述，单侧 95% 上界为
0.88%。该区间忽略同患者内聚类，只用于技术稳定性描述，不能解释为临床错误率。

混淆矩阵（行 Qwen3.6，列 Qwen3.8）：

| Qwen3.6 \ Qwen3.8 | V | I | C |
|---|---:|---:|---:|
| V | 1 | 1 | 0 |
| I | 0 | 8 | 1 |
| C | 0 | 5 | 324 |

完全一致 **333/340 = 97.94%**，Cohen's κ=**0.722**。7 个分歧规则为：

| 规则 | Qwen3.6 | Qwen3.8 | 解释口径 |
|---|---|---|---|
| R141 | V / 0.90 | I / 0.55 | 3.8 丢失一个基线 V，最高优先级专家复核 |
| R146 | C / 0.90 | I / 0.55 | 3.8 更保守 |
| R224 | C / 0.95 | I / 0.55 | 3.8 更保守 |
| R161 | C / 0.50 | I / 0.55 | 两侧均处边界 |
| R103 | C / 0.95 | I / 0.50 | 3.8 更保守 |
| R154 | C / 0.90 | I / 0.55 | 3.8 更保守 |
| R020 | I / 0.00 | C / 0.92 | 3.6 零工具截断；3.8 正常 3 tools / 21.1s |

本次 3.8 没有新增 V，但把 1 个基线 V 改成 I、5 个 C 改成 I。没有专家金标时，不能把
更保守的 I 自动解释为更安全，也不能把 97.94% 的模型间一致率解释为 97.94% 临床准确率。
最小专家集就是上表 7 对，尤其先看 R141；R020 是明确的 3.6 技术失败，可与其他 6 对分开。

### 0.4 与先前两患者复测合并

按 `patient + batch_tag + rule + model` 取 `ab3.8` 最新结果，累计为 12 患者、425 个完整
配对、0 missing，完全一致 **394/425 = 92.71%**。累计矩阵为：
...
新增十患者以 CLEAN 为主，因此累计一致率显著高于先前两患者的 70.6%；这说明一致率受病例
谱和类别流行率影响很大。专家判断应优先看所有 V 迁移和分歧卡片，不应只看总体一致率。

**最新建议：JSON/工具协议适配已通过，可用 `ab3.8` Workbench 双模型页开展专家盲审；
Qwen3.8 工具调用少 23.2%，但墙钟仍慢约 3.87 倍，且存在 R141 的 V→I。生产继续保持
Qwen3.6，至少完成 7 个新分歧和累计历史分歧的专家裁决后再讨论切换。**

## 0A. 2026-08-19 原生工具协议适配复测（历史）

上一轮失败的主要混杂变量已经消除：Javert 现在可通过
`JAVERT_LLM_TOOL_PROTOCOL=native` 把现有 12 个工具 JSON Schema 作为 OpenAI `tools`
传给 Qwen3.8，并消费 SGLang `message.tool_calls`；工具结果按 `role=tool` 回灌。原生模式还
执行相同参数去重、同工具最多 3 次、每规则总调用最多 10 次，预算耗尽时移除 tools Schema，
强制输出最终 fenced JSON。现网 Qwen3.6 继续使用默认 `text` 协议，不受影响。

真实门禁与复测结果：

- 合成两轮 function-call：`tool_calls → role=tool → DONE`，两次 HTTP 200。
- 真实单规则 dry-run：6 次工具、72.2 秒、0 解析错误，正常收敛
  `VIOLATION/confidence=0.88`。
- 正式批次统一使用 `batch_tag=ab3.8`、并发 2、thinking off、temperature 0、
  drift guard off；Qwen3.6 与 Qwen3.8 各完成 85/85，全部同步 142，0 failed、0 timeout、
  0 malformed。
- 适配后的 Qwen3.8 工具调用总数为 322，低于 Qwen3.6 的 391；不再出现单规则 275 次
  调用或协议导致的 912 秒失败。
- Qwen3.8 的累计规则时长为 2126.1 秒，Qwen3.6 为 574.2 秒，仍慢 3.70 倍；这与 dense
  模型的纯推理差距同量级，协议额外放大基本消失。

| 病例 | 规则 | Qwen3.6 V/I/C | Qwen3.8 V/I/C | 3.6 工具 | 3.8 工具 | 3.6 累计规则时长 | 3.8 累计规则时长 |
|---|---:|---|---|---:|---:|---:|---:|
| Case-A | 30 | 0/1/29 | 0/4/26 | 123 | 107 | 192.2s | 703.1s |
| Case-B | 55 | 1/1/53 | 3/19/33 | 268 | 215 | 382.0s | 1423.0s |
| **合计** | **85** | **1/2/82** | **3/23/59** | **391** | **322** | **574.2s** | **2126.1s** |

最新混淆矩阵（行 Qwen3.6，列 Qwen3.8）：

| Qwen3.6 \ Qwen3.8 | V | I | C | MISSING |
|---|---:|---:|---:|---:|
| V | 1 | 0 | 0 | 0 |
| I | 0 | 1 | 1 | 0 |
| C | 2 | 22 | 58 | 0 |

完全一致 60/85 = 70.6%，25 条需专家判断。Qwen3.8 在本批没有丢失 Qwen3.6 唯一的 V，
但新增 2 个 V、把 22 个 C 改为 I，并把 1 个 I 改为 C。由于没有专家金标，这些迁移不能
自动解释为提升或退化。

Workbench 已增加登录保护的双模型页面：进入 `ab3.8` tag 的患者详情，点击
“↔ Qwen3.6 / 3.8 对比”，即可按同患者、同规则并排查看裁决、置信度、模型推理、证据、
工具链和耗时。页面直接按 `patient + batch_tag + rule + model` 从 142 配对，不依赖历史卡片
顺序；访问写入 `model_compare_access` 审计日志。

**最新建议：协议适配已经通过，可进入专家盲审；生产仍保持 Qwen3.6，等 25 条分歧完成
专家裁决后再决定是否切换。**
