...
def backfill_sqlite(db_path, loader, meta_map, kb_drugs, *, dry_run=False, limit=None):
    con = sqlite3.connect(str(db_path))
    try:
        _ensure_sqlite_column(con)
        sql = (
            "SELECT run_id, rule_id, patient_id, evidence_json, tool_calls_json "
            "FROM audit_runs ORDER BY run_id"
        )
    ...
    with engine.connect() as conn:
        ...
        for r in rows:
            ...
            if len(batch) >= batch_size:
                flush(batch)
                logger.info("142 backfill: %d/%d 已提交", written, total)
                batch = []
        flush(batch)  # 余量
