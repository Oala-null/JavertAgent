...
**专家漏检线索补充（2026-09-06，本地 ready，待手测）**：补强 R319–R322 的别名与证据边界，新增 R323–R326、RD38，覆盖眼科手术分解/执行、眼内能量治疗数量、穿刺与球后/球旁注射互斥、中医治疗执行和联合滴眼用药必要性。本次仅编写及静态校验，未跑测试或病例、未部署；当地价格条文和执行证据不足时保留待核查。详见[规则覆盖与人工验收清单](docs/expert_ophthalmology_rules.md)。

**上述规则已发布62（2026-09-07 UTC）**：R326完成一份授权PDF的脱敏定向审计，三项中医治疗均命中，结论待人工复核；结果在工作台tag“眼科”。修复行为分类启动校验及CSV收费编码前导零问题；相关组合180 passed / 1 skipped，生产版本/配置/SQL/Hub/v3门禁通过。其他八条仍需对应真实病例验证，详见[验收记录](docs/expert_ophthalmology_rules_qa.md)。
