...
> 版本：v1.1 ｜ 2026-07-17
>
> **运行时更新（2026-07-17）**：62 当前模型为
> `Qwen3.6-35B-A3B-FP8`；Javert 当前规则为 159 条、其中 118 条
> production-ready。本文 §5.1.1 的 Qwen3.5/GPTQ 显存拆解保留为历史测量，不用于当前采购定型。
...
flowchart LR
    ...
    subgraph BOX["院内 GPU 服务器（数据不出院）"]
        E["Qwen3.6 FP8 推理引擎<br/>调查员 Agent"]
...
肿瘤药医保限定规则 `RD04` 还会输出两条互不替代的结构化轴：
审计处置 `VIOLATION_FOUND / REVIEW_REQUIRED / NO_VIOLATION_FOUND` 与资格状态
`SATISFIED / NOT_SATISFIED / DOCUMENTATION_GAP / CONFLICT`，并附条件树、证据树和数据质量提示。
...
flowchart LR
    ...
    subgraph RULES["规则库（我方自带）"]
        R["159 条规则<br/>118 条 production-ready"]
    end
    subgraph BOX["院内 GPU 服务器（数据不出院）"]
        RT["规则路由<br/>砍 70-80% 无关规则"]
        E["Qwen3.6 FP8 调查员 Agent<br/>+ 审计工具组"]
        G["确定性闸<br/>病案首页兜底校验"]
        RT --> E --> G
    end
    A --> RT
...
两个产品**共用同一套院内 GPU 环境**。62 当前端点部署
`Qwen3.6-35B-A3B-FP8`；对接一台机器即可同时支撑两个产品。模型版本和量化方式会随
验证结果升级，采购前应按目标版本重新压测，不能沿用旧模型显存估算。
...
> **历史基线**：本节以下参数与公式对应 2026-06-10 的
> `Qwen3.5-35B-A3B-GPTQ-Int4` 环境，只解释当时 48GB 卡的占用，不代表
> 2026-07-17 的 Qwen3.6 FP8 运行时。
